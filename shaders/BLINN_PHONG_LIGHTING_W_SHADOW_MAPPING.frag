...
float Calculate_Shadows(vec4 light_space_model_fragments, vec3 direction_lighting_arg,  vec3 normal_coordinates_arg);
...
vec3 DirectionalLightingCalculations (DirectionalLighting direction_lighting_arg, vec3 normal_coordinates_arg, vec3 pos_of_camera_arg, vec4 light_fragments, vec3 model_frags);
...
void main()
{
	...
	vec3 lighting_calculations = DirectionalLightingCalculations(directional_lighting_obj, normalized_normal_coordiantes, normalized_direction_of_camera_from_the_model, position_of_model_fragments_from_light_pov, position_of_model_fragments);
	...
	/*
	vec3 projection_coords = position_of_model_fragments_from_light_pov.xyz / position_of_model_fragments_from_light_pov.w;

	projection_coords = projection_coords * 0.5 + 0.5;

	float depth_sample = texture(shadowDepthMapTexture, projection_coords.xy).r;

	color_output = vec4(vec3(depth_sample), 1.0);
	*/
...
vec3 DirectionalLightingCalculations (DirectionalLighting direction_lighting_arg, vec3 normal_coordinates_arg, vec3 pos_of_camera_arg, vec4 light_fragments, vec3 model_frags)
{
	...
	float in_shadow = Calculate_Shadows(light_fragments, light_direction, normal_coordinates_arg);
...
float Calculate_Shadows(vec4 light_space_model_fragments, vec3 direction_lighting_arg, vec3 normal_coordinates_arg)
{
	...
	//vec3 norm_light_dir = normalize(-direction_lighting_arg.light_direction);

	float bias = max(0.05 * (1.0 - dot(normal_coordinates_arg, direction_lighting_arg)), 0.005);
	...
	//float in_shadow = current_depth_value_from_light_perspective - bias > closest_depth_value ? 1.0 : 0.0;
	...
	if (current_depth_value_from_light_perspective > 1.0)
	{
		in_shadow = 0.0;
	}
	...
	vec2 size_of_texel = 1.0 / textureSize(shadowDepthMapTexture, 0);
	for (int x_coordinate = -1; x_coordinate <= 1; ++x_coordinate)
	{
		for(int y_coordinate = -1; y_coordinate <= 1; ++y_coordinate)
		{
			float depthPCF = texture(shadowDepthMapTexture, light_space_projection_coordinates.xy + vec2(x_coordinate, y_coordinate) * size_of_texel).r;
			in_shadow += current_depth_value_from_light_perspective - bias > depthPCF ? 1.0 : 0.0;
		}
	}
	in_shadow /= 9.0;
	*/
