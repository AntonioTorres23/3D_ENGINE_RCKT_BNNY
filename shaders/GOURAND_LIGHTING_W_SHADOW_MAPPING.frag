...
void main()
{
	vec3 ambient = ambient_return * texture(diffTex1, tex_coords).rgb;
	vec3 diffuse = diffuse_return * texture(diffTex1, tex_coords).rgb;
	vec3 specular = specular_return * texture(diffTex1, tex_coords).rgb;
