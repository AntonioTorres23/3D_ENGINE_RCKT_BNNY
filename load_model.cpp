...
MODEL_OBJ::MODEL_OBJ(std::string path_to_3D_model_filetype, std::string model_name, bool gam) : gammaCor(gam)
{
...
void MODEL_OBJ::Model_Load(std::string path_to_3D_model_filetype, std::string model_path_name)
{
...
void MODEL_OBJ::Parse_Node(aiNode* model_node, const aiScene* model_scene)
{
...
MESH_OBJ MODEL_OBJ::Parse_Mesh(aiMesh* model_mesh, const aiScene* model_scene)
{
...
std::vector<tData_Struct> MODEL_OBJ::Load_Textures_From_Materials(aiMaterial* model_material, aiTextureType tex_type, std::string tex_type_name)
{
	...
	for (unsigned int texture = 0; texture < model_material->GetTextureCount(tex_type); texture++)
	{
		...
		if (!skip_tex)
		{
			...
			material_tData.texID = Textures_In_Model_Directory(assimp_string.C_Str(), this->model_files_directory, this->gammaCor);
